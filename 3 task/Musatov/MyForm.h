...
// № 1 (x,y) (x,y, 1.0)
// Однородные координаты точки = координаты, обладающие тем свойством что
//определяемый ими объект не меняется когда все координаты умножаются на одно и то же число

// № 2 В Matrix.h

// № 3 Поменяет своё направление

// № 4 initT матрица начального преобразованияч

// № 5 T - матрица последующих преобразований, изначально единичная

// № 6 в переносе начала координат в середину окна, повороте и обратном переносе. см тетрадь


/* № 7	mirrorY, меняем на отрицательные
case Keys::U:
T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
T = mirrorX() * T;		//Зеркальное отображение относительно горизонтальной оси
// нового центра против часовой стрелки
...
// № 8 Если вместо масштабирования относительно центра окна
// использовать элементарные преобразования масштабирования, то
// изображение может исказиться. Это зависит от коэффициентов
// масштабирования: если они не равны, то объект будет искажён

// № 9 числа от 0 до 255, меняющие цвет

// № 10 Color.FromArgb Method метод https://learn.microsoft.com/en-us/dotnet/api/system.drawing.color.fromargb?view=net-8.0

// № 11 от файла txt. color